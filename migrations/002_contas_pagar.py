"""
Migração 002: Criação das tabelas de Contas a Pagar
"""
...
def upgrade(db):
    ...
    with db.get_connection() as conn:
        ...
        cursor.execute(CREATE_ANEXOS_CONTAS_PAGAR)
        conn.commit()
...
def downgrade(db):
    ...
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DROP TABLE IF EXISTS anexos_contas_pagar")
        cursor.execute("DROP TABLE IF EXISTS contas_pagar")
        conn.commit()
        print("✓ Tabelas de Contas a Pagar removidas")
